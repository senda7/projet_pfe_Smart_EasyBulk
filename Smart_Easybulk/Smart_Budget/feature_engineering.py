...
──────────────────────────────────────────────────────────────────
Transforme les données nettoyées en table d'apprentissage.
Une ligne = un groupe × une semaine.

CORRECTIONS APPLIQUÉES (vs ancien feature engineering.py) :
  C1  Quota reconstruit depuis budget_history (plus de fuite)
  C2  Année ISO cohérente avec numero_semaine ISO
  M2  Ratio d'accélération = 0 quand pas d'historique (évite valeurs fantaisistes)
  M3  Découpage hebdomadaire en timezone Africa/Tunis (UTC+1)

AMÉLIORATIONS APPORTÉES :
  Approche C  Historique saisonnier par fenêtre calendaire ±7 jours
              (à la place du matching par semaine ISO)
  B.1  Features "prochain événement"    : mult + jours avant
  B.2  Features "rythme d'épuisement"   : jours avant quota=0 + accélération

ENTRÉE  : output/clean_groupes.json
          output/clean_campagnes.json
          output/clean_budget_history.json
          output/events_calendar.json

SORTIE  : output/features.csv
          output/feature_cols.json

FAMILLES DE FEATURES (10 au total — jeu réduit, Option B)
  [CONSO]     2  — conso_moyenne_4sem, conso_semaine_1
  [TENDANCE]  1  — tendance_ratio_accel
  [CAMPAGNE]  1  — campagne_nb_30j   (fenêtre rétrospective 30 jours)
  [BUDGET]    2  — budget_quota_libre, budget_ratio_libre
  [SAISON]    2  — saison_multiplicateur, saison_conso_historique_semaine
  [PROCHAIN]  1  — evenement_prochain_jours
  [RYTHME]    1  — jours_avant_zero_rythme_event

Note importante (test post-audit) : nous avons testé un retrait des features
"marginales" (tendance_ratio_accel et evenement_prochain_jours, importance
< 1 %). Résultat : la précision CRITIQUE a chuté de 82 % à 69 %.
Conclusion : l'importance d'une feature n'est pas un indicateur déterministe
de son utilité — sur un petit jeu de test, une feature peu utilisée dans
le modèle global peut faire basculer 1-2 cas borderline.
On garde donc les 10 features.

Variables internes (pas dans le modèle) : budget_quota_total, evenement_prochain_mult
  → nécessaires au calcul d'autres features mais redondantes en tant que prédicteurs.
"""
